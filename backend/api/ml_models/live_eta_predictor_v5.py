...
class LiveETAPredictorV5:
    ...
    def __init__(
        ...
        self.stops = stops.copy().sort_values("stop_number").reset_index(drop=True)
        ...
        if direction == "forward":
            ...
            raise ValueError("direction must be 'forward' or 'return'")

        self.resample_tolerance_sec = float(resample_tolerance_sec)
    ...
    def next_stop_info(self):
        ...
        match = self.stops[self.stops["stop_number"] == self.current_next_stop_number]
    ...
    def _nearest_history_point(self, target_time, max_error_sec=4.0):
        ...
        errors = (times - target_time).abs().dt.total_seconds()
    ...
    def _window(self, current_time, seconds):
        ...
        window = self.history[
            (self.history["time"] >= start) & (self.history["time"] <= current_time)
        ]
    ...
    def _build_features(self, current_time, next_stop):
        ...
        point, endpoint_error = self._nearest_history_point(
            current_time, max_error_sec=self.resample_tolerance_sec
        )
        ...
        p30, error30 = self._nearest_history_point(
            actual_time - pd.Timedelta(seconds=30), max_error_sec=4.0
        )

        p60, error60 = self._nearest_history_point(
            actual_time - pd.Timedelta(seconds=60), max_error_sec=4.0
        )

        if p30 is None or p60 is None:
            return None

        window30 = self._window(actual_time, 30)

        window60 = self._window(actual_time, 60)
        ...
        rolling_speed_30s = float(window30["speed_mps"].mean())

        rolling_speed_60s = float(window60["speed_mps"].mean())

        dist30 = haversine_live_v5(
            p30["latitude"], p30["longitude"], point["latitude"], point["longitude"]
        )

        elapsed30 = (actual_time - p30["time"]).total_seconds()
        ...
        dist60 = haversine_live_v5(
            p60["latitude"], p60["longitude"], point["latitude"], point["longitude"]
        )

        elapsed60 = (actual_time - p60["time"]).total_seconds()
        ...
        is_stopped = float(speed <= 0.5)

        is_very_slow = float(speed <= 2.0)

        is_slow = float(speed <= 5.0)

        is_stopped_30s = float(progress_speed_30s <= 0.5)

        is_very_slow_30s = float(progress_speed_30s <= 2.0)

        is_slow_30s = float(progress_speed_30s <= 5.0)

        is_stopped_60s = float(progress_speed_60s <= 0.5)

        is_very_slow_60s = float(progress_speed_60s <= 2.0)

        is_slow_60s = float(progress_speed_60s <= 5.0)
        ...
        values = np.array([features[f] for f in LIVE_FEATURES_V5], dtype=np.float32)
    ...
    def _scale_sequence(self, sequence):
        ...
        continuous = sequence[:, CONTINUOUS_INDICES_V5]

        continuous_scaled = self.scaler.transform(continuous)

        sequence[:, CONTINUOUS_INDICES_V5] = continuous_scaled

        if sequence.shape != original_shape:
            raise RuntimeError("Scaling changed sequence shape.")
    ...
    def update(
        ...
        cutoff = timestamp - pd.Timedelta(seconds=self.history_keep_sec)

        self.history = self.history[self.history["time"] >= cutoff].reset_index(
            drop=True
        )
        ...
        history_seconds = (
            self.history["time"].max() - self.history["time"].min()
        ).total_seconds()
        ...
        if history_seconds < 60:
            ...
            return {
                ...
                "next_stop_number": int(next_stop["stop_number"]),
        ...
        for offset in offsets:
            ...
            feature_time = timestamp + pd.Timedelta(seconds=int(offset))
            ...
            if result is None:
                ...
                return {
                    "status": "feature_history_not_ready",
                    "timestamp": timestamp,
                    "history_seconds": float(history_seconds),
                    "next_stop_number": int(next_stop["stop_number"]),
                    "next_stop": next_stop["stop_name"],
        ...
        eta_seconds = float(prediction.reshape(-1)[0])
        ...
        return {
            ...
            "history_seconds": float(history_seconds),
            "max_resample_error_sec": (float(max(errors)) if len(errors) else 0.0),
        }
