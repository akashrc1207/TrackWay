...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--bus', type=str, default='Nayana', help='Bus name to simulate')
        parser.add_argument('--interval', type=float, default=4.0, help='Seconds between GPS updates')
        parser.add_argument('--step', type=int, default=5, help='Polyline points per step')
        parser.add_argument('--reset', action='store_true', help='Reset old GPS logs before starting')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f" [BUS] TRACKWAY PERFECT LIVE DEMO SIMULATOR ({bus_name.upper()})"))
        self.stdout.write(self.style.SUCCESS("=" * 60))
        ...
        if not bus:
            ...
            if not route:
                self.stdout.write(self.style.ERROR("No route found. Please run seed_data first."))
                return
        ...
        if should_reset or GPSLog.objects.filter(bus=bus).count() > 500:
            ...
            self.stdout.write(self.style.NOTICE(f"Cleaned up {deleted_count} stale GPS logs for {bus.bus_name}."))
        ...
        Journey.objects.filter(bus=bus, is_active=True).update(is_active=False, end_time=timezone.now())
        ...
        self.stdout.write(self.style.SUCCESS(f"Active Journey Created: Session #{journey.id} for {bus.bus_name} ({bus.bus_number})"))
        ...
        if os.path.exists(polyline_path):
            with open(polyline_path, "r") as f:
                points = json.load(f)
            self.stdout.write(self.style.SUCCESS(f"Loaded {len(points)} dense road polyline coordinates."))
        else:
        ...
        if not points:
            self.stdout.write(self.style.ERROR("No coordinates available for simulation."))
            return
        ...
        self.stdout.write(self.style.SUCCESS(f"Broadcasting live GPS stream for {bus.bus_name}... Press Ctrl+C to stop.\n"))
        ...
        try:
            while True:
                ...
                dir_str = "Forward (Thaliparamba -> Cherupuzha)" if direction == 1 else "Return (Cherupuzha -> Thaliparamba)"
